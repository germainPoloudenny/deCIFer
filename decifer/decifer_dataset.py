...
class DeciferDataset(Dataset):
    ...
    def _build_conditioning_cache(self):
        ...
        if tqdm is not None:
            ...
            batch_iter = tqdm(batch_iter, total=total_batches, desc="Precomputing XRD cache", leave=False)
        ...
        for start in batch_iter:
            ...
            batch_q = torch.from_numpy(batch_q_np)
            batch_iq = torch.from_numpy(batch_iq_np)

            with torch.no_grad():
                result = discrete_to_continuous_xrd(batch_q, batch_iq, **self.conditioning_kwargs)

            cache.append(result['iq'].clone().detach())
